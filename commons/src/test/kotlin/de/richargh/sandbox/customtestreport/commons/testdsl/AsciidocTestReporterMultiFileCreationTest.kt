...
@ReportAsAsciiDoc
class AsciidocTestReporterMultiFileCreationTest {

    @Test
    fun `will create report file for this test`(
        testEvents: StructuredTestEvents, @AsciiDocReportDirectory reportDirectory: Path
    ) {
        /** empty because asserts are in after **/
    }

    @Test
    fun `will also create report file for this test`(
        testEvents: StructuredTestEvents, @AsciiDocReportDirectory reportDirectory: Path
    ) {
        /** empty because asserts are in after **/
    }
    ...
    fun after(@AsciiDocReportDirectory reportDirectory: Path, testInfo: TestInfo) {
        val expectedTestDirectory = reportDirectory.resolve(this::class.simpleName!!)
        val expectedReportFile = expectedTestDirectory.resolve("${testInfo.displayName}.adoc")
        ...
        assertThat(actualTestDirectory.listFiles()).contains(expectedReportFile.toFile())
    }
}
